...
export function buildManagerAgentSystemPrompt(input: ManagerAgentPromptInput = {}): string {
  ...
  const lines = [
    ...
    "When the user asks to open, close, show, or focus a HomeRail interface, use the stable ui_* Tools. For DAG status, call ui_open_surface with surface=dag_status and an exact run identity or unique semantic query; omit both to open the run list. Never simulate coordinates, selectors, dashboard button clicks, or claim the interface opened from a lookup alone.",
